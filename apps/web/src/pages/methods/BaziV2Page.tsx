...
export function BaziV2Page() {
  ...
  const cfg = METHOD_INPUT_CONFIG.bazi_v2;
  ...
  const inBasket = useBasket((s) => s.has("bazi_v2"));
  ...
  const submit = useCallback(async (e: FormEvent) => {
    ...
    try {
      const b = buildBirth();
      const chart = await computeChart({ method: "bazi_v2", birth: b, options: { mode, question: question || undefined } });
      storeAndNavigate("bazi_v2", b, [chart]);
    } catch (err: any) { setError(String(err?.message || err)); }
    finally { setLoading(false); }
  }, [birth, mode, question, navigate, cityInfo]);
  ...
  function storeAndNavigate(method: any, b: Birth, charts: ChartResult[]) {
    ...
    useHistory.getState().add({
      ...
      subject: "self_life" as any, modeByMethod: { [method]: mode },
  ...
  function addToBasket() {
    basketAdd({ method: "bazi_v2", chart: null, birth: buildBirth(), addedAt: Date.now() });
  }

  return (
    <form onSubmit={submit} className="space-y-5">
      <header>
        <h1 className="paper-title"><span className="stamp" />{t("method.bazi_v2.title")}</h1>
        <p style={{ fontSize: "0.83rem", color: "var(--ink-soft)", marginTop: "0.4rem", lineHeight: 1.6 }}>{t("method.bazi_v2.desc")}</p>
      </header>
