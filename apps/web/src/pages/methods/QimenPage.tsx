...
export function QimenPage() {
  ...
  const cfg = METHOD_INPUT_CONFIG.qimen;
  ...
  const submit = useCallback(async (e: FormEvent) => {
    ...
    try {
      ...
      storeAndNavigate("qimen", b, [chart]);
  ...
  function storeAndNavigate(method: any, b: Birth, charts: ChartResult[]) {
    ...
    useHistory.getState().add({ id: hid, ts: Date.now(), birth: b, methods: [method], charts: { [method]: charts[0] }, question: question || undefined, subject: "decision" as any, modeByMethod: { [method]: mode }, tags: deriveTags([method], "decision"), favorite: false, reflection: null });
  ...
  function addToBasket() { basketAdd({ method: "qimen", chart: null, birth: buildBirth(), addedAt: Date.now() }); }

  return (
    <form onSubmit={submit} className="space-y-5">
      <header><h1 className="paper-title"><span className="stamp" />{t("method.qimen.title")}</h1><p style={{ fontSize: "0.83rem", color: "var(--ink-soft)", marginTop: "0.4rem", lineHeight: 1.6 }}>{t("method.qimen.desc")}</p></header>
      ...
      <MethodSubmitBar loading={loading} error={error} inBasket={inBasket} onAddToBasket={addToBasket} submitLabel={lang === "zh" ? "排奇门盘" : "Cast Qi Men"} />
