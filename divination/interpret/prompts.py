...
def _serialize(c: ChartResult) -> str:
    ...
    if m == "hepan":
        h = r["八字合婚"]; w = r["西方合盘"]
        key = [f"{c['A']}-{c['B']}{c['相位']}" for c in w["关键相位"][:6]]
        return (f"【合盘·中西合参】八字：日干{h['日柱']['日干']['关系']}({h['日柱']['日干']['义']})，"
                f"日支{h['日柱']['日支']['关系']}，鸳鸯合={h['鸳鸯合']}，"
                f"属相{h['年支(属相)']['属相']}{h['年支(属相)']['关系']}，"
                f"用神互补：B补A{h['用神互补']['B补A']}/A补B{h['用神互补']['A补B']}。"
                f"西方跨盘：谐和{w['谐和数']}/张力{w['张力数']}，关键相位{key}。"
                f"维度评级：{r['维度评级']}。"
                + (r['印证'] or r['分歧'] or ""))
    if m == "numerology":
